...
7. **LLM API keys are never hardcoded or committed.** Load from an environment variable (e.g. `GEMINI_API_KEY`) via `.env` (gitignored) — commit only `.env.example` with a placeholder. If a key is missing, fail with a clear message telling the user to set it; don't silently disable the feature or fall back to fabricated output.
8. **LLM-generated content is always labeled as what it is.** Natural-language training configs and match commentary (Stages 37-38) must be visibly presented as AI-generated interpretation — never implied to be the robot's literal internal reasoning, since the actual policy (PPO or a small evolved network) has no language-based cognition at all. Getting this framing wrong is the same category of problem as faking a result: it misrepresents what the system is actually doing.
